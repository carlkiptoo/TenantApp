...
//     final prefs = await SharedPreferences.getInstance();
//     final tenantId = await prefs.getString('tenantId');
//
//     if (tenantId == null) {
...
//       final rentStatusList = json.decode(response.body);
...
//                   child: ListTile(
//                     title: Text(
//                         'Unit Number: ${latest['tenantId']['unitNumber'] ??
//                             'N/A'}'),
...
//                       children: [
//                         _buildSummaryRow(
//                             'Monthly Rent', 'KES ${latest['rentAmount'] ?? 0}'),
//                         _buildSummaryRow(
//                             'Water Bill', 'KES ${latest['waterBill'] ?? 0}'),
...
//
//                         _buildSummaryRow(
//                             'Total Due', 'KES ${(latest['rentAmount'] ?? 0) +
//                             (latest['waterBill'] ?? 0) + 200}',
...
//                     entry['month'] ?? 'Unknown',
//                     'KES ${(entry['rentAmount'] ?? 0) +
//                         (entry['waterBill'] ?? 0) + 200}',
//                     entry['status'] ?? 'Unpaid',
...
//       String datePaid) {
//     Color statusColor =
//     status == 'Paid' ? Colors.green : status == 'Unpaid' ? Colors.red : Colors
//         .orange;
//
//     return Card(
//       child: ListTile(
                ...
                children: [
                  ...
                  Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          _buildSummaryRow('Monthly Rent', 'KES ${latest['rentAmount'] ?? 0}'),
                          _buildSummaryRow('Water Bill', 'KES ${latest['waterBill'] ?? 0}'),
                          _buildSummaryRow('Garbage Fee', 'KES $garbageFee'),
                          _buildSummaryRow('Other Charges', 'KES $otherCharges'),
                          const Divider(),
                          _buildSummaryRow(
                            'Total Due',
                            'KES ${(latest['rentAmount'] ?? 0) + (latest['waterBill'] ?? 0) + garbageFee + otherCharges}',
                            isBold: true,
                            color: Colors.red,
                          ),
                          const SizedBox(height: 6),
                  ...
                  ...rentStatus.map((entry) {
                    return _buildPaymentTile(
                      entry['month'] ?? 'Unknown',
                      'KES ${(entry['rentAmount'] ?? 0) + (entry['waterBill'] ?? 0) + garbageFee + otherCharges}',
                      entry['status'] ?? 'Unpaid',
                      _formatDate(entry['paidDate']),
                    );
                  }).toList(),
